...
pub async fn translate_text(config: &Config, text: &str) -> Result<String> {
    let provider = config.ai_provider.as_ref()
        .ok_or_else(|| AppError::InternalError(
            "AI翻译功能未启用。请在环境变量中设置 AI_PROVIDER (ollama 或 openai)".to_string()
        ))?;
...
pub async fn translate_with_openai(config: &Config, text: &str) -> Result<String> {
    let api_key = config
        .openai_api_key
        .as_ref()
        .ok_or_else(|| AppError::InternalError("未配置 OpenAI API Key。请设置 OPENAI_API_KEY 环境变量".to_string()))?;
    
    let api_base = config
        .openai_api_base
        .as_ref()
        .ok_or_else(|| AppError::InternalError("未配置 OpenAI API Base。请设置 OPENAI_API_BASE 环境变量".to_string()))?;
    
    let model = config
        .openai_model
        .as_ref()
        .ok_or_else(|| AppError::InternalError("未配置 OpenAI Model。请设置 OPENAI_MODEL 环境变量".to_string()))?;
...
pub async fn extract_tags_from_content(
    ...
    let provider = config.ai_provider.as_ref()
        .ok_or_else(|| AppError::InternalError(
            "AI功能未启用，无法自动提取标签".to_string()
        ))?;

    let tags_text = match provider.as_str() {
    ...
    let response = client
        ...
        .map_err(|e| {
            if e.is_timeout() {
                ...
                AppError::ExternalApiError(format!("无法连接到 Ollama 服务 ({})", ollama_url))
            } else {
